globalThis.ItemEditor = {
    ...
    switchTab(tabNum) {
        this._currentTab = tabNum;
        document.querySelectorAll('.item-tab-panel').forEach(p => {
            p.classList.toggle('hidden', parseInt(p.dataset.tab, 10) !== tabNum);
        });
        document.querySelectorAll('.item-tab-btn').forEach(btn => {
            const isActive = parseInt(btn.dataset.tab, 10) === tabNum;
            btn.classList.toggle('bg-purple-500/80', isActive);
    ...
    _populateCommandBar() {
        ...
        <div class="flex items-center justify-between gap-3 flex-wrap">
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <div id="dirty-indicator" class="hidden items-center gap-1 bg-amber-900/30 border border-amber-500/30 rounded-lg px-2.5 py-1">
                    <i class="fa-solid fa-circle-dot text-amber-400 text-[10px] animate-pulse"></i>
                    <span class="text-[8px] font-black uppercase text-amber-400 tracking-wider">NIEZAPISANE</span>
                </div>
            </div>
            <div class="flex items-center gap-2">
    ...
    _populateAnchorNav() {
        // No-op — replaced by tabbed editor (Tab 1-7). See §3-§9 design doc.
    },
    ...
    _glassCard(id, color, icon, title, content) {
        return `
        <section id="${id}" class="studio-card p-5 relative overflow-hidden transition-all hover:border-white/12">
            <div class="absolute top-0 left-0 w-1 h-full bg-${color}-500"></div>
            <h3 class="text-[11px] font-black uppercase text-${color}-400 tracking-widest mb-4 flex items-center gap-2 pl-2">
    ...
    _inp(id, label, placeholder, extra = '') {
        return `<div class="flex flex-col gap-1.5 ${extra}">
            ...
            <input type="text" id="${id}" class="studio-input px-3 py-2.5 text-[13px]" placeholder="${placeholder}">
        </div>`;
    },
    ...
    ensureOmnichannelForm() {
        ...
        // Faza 4: inspector visibility managed by Core.updateInspector()
        ...
        const TABS = [
            { n: 1, icon: 'fa-fingerprint',   label: 'Podstawowe' },
            { n: 2, icon: 'fa-table-cells',    label: 'Ceny' },
            { n: 3, icon: 'fa-puzzle-piece',    label: 'Dodatki' },
            { n: 4, icon: 'fa-clapperboard',   label: 'Scena' },
            { n: 5, icon: 'fa-truck-fast',      label: 'Logistyka' },
            { n: 6, icon: 'fa-calendar-days',  label: 'Czas' },
        ...
        form.innerHTML = `
            ...
            <!-- ═══ Tab 1: Podstawowe (Identity + Variants + Description + Badge + Hero) ═══ -->
            <div class="item-tab-panel" data-tab="1">
                ...
                ${this._glassCard('sec-desc-badge', 'rose', 'fa-bullhorn', 'Opis i Odznaka', `
            ...
            <!-- ═══ Tab 3: Dodatki (Modifiers) ═══ -->
            <div class="item-tab-panel hidden" data-tab="3">
            ...
            <!-- ═══ Tab 4: Scena (Visual) ═══ -->
            <div class="item-tab-panel hidden" data-tab="4">
            ...
            <!-- ═══ Tab 5: Logistyka ═══ -->
            <div class="item-tab-panel hidden" data-tab="5">
            ...
            <!-- ═══ Tab 6: Czas (Schedule) ═══ -->
            <div class="item-tab-panel hidden" data-tab="6">
            ...
            <!-- ═══ Tab 7: Marketing (Tags + Allergens + EAN + Parent SKU + Secret + Legacy URL) ═══ -->
            <div class="item-tab-panel hidden" data-tab="7">
                ${this._glassCard('sec-marketing', 'rose', 'fa-tags', 'Tagi i Marketing', `
                    <div id="lock-item-tags-wrapper" class="flex flex-col gap-1.5 franchise-lockable">
                ...
                ${this._glassCard('sec-enterprise', 'emerald', 'fa-bolt', 'Enterprise Settings (Retail & Warianty)', `
                    ...
                    <div class="mt-4 flex items-center gap-2.5 cursor-pointer">
                        <label class="flex items-center gap-2.5 cursor-pointer group">
                            <div class="relative">
    ...
    async duplicateItem() {
        ...
        try {
            ...
            const src = res.data;
            const newKey = (src.asciiKey || asciiKey).substring(0, 42) + '_K';
            const newName = (src.name || '') + ' (kopia)';
