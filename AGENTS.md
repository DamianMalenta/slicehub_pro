...
16. **Timezone SSOT (2026-08-25):** `core/db_config.php` ustawia `date_default_timezone_set('Europe/Warsaw')` globalnie (includowane przez 97 plików). Wcześniej brak → PHP używał ini `date.timezone` (XAMPP: `Europe/Berlin`, Cloud: często `UTC`), co powodowało 2h przesunięcia między `PromisedTimeEngine` (hardcoded Warsaw) a fallbackami `date('Y-m-d H:i:s')` w POS/tables/online. Frontend wysyła naive ISO `2026-08-25T15:30` (bez offsetu) — działa bo `strtotime` interpretuje jako Warsaw local (spójne z browser tz). Gdyby PHP tz ≠ browser tz, trzeba wysyłać pełny ISO z `+02:00`.
17. **promised_time = NULL = ASAP (neutralny, 2026-08-25):** `pos_ui.js fmtTime()` zwraca białe "ASAP" gdy `promised_time` jest NULL — **NIE** fallback na `created_at` (które jest zawsze w przeszłości → ujemny diff → karta świeci na czerwono od razu po utworzeniu). `buildCard()` przekazuje TYLKO `o.promised_time` (bez `|| o.created_at`). `tables/engine.php` i `seed_demo_all.php` wstawiają `promised_time` przez `PromisedTimeEngine::calculate('asap', channel)` przy tworzeniu — wcześniej NULL. `shift_time` z `delay_minutes` zwraca `resolved_promised: null` (per-order, SQL używa `DATE_ADD(COALESCE(promised_time, created_at), INTERVAL :delay)`) + `delay_minutes` w response.
18. **Countdown tick (2026-08-25):** `pos_app.js` odpala `_tickTimer` co 30s (`TICK_INTERVAL`) wywołujący `PosUI.tickCountdowns(orders)` — lekka aktualizacja `.time-rel` (Kanban), `.pulse-countdown` + `.pulse-time` (Pulse) i SLA class (`sla-green/yellow/red`) bez pełnego re-renderu. Eliminuje "zamrożenie" czasu między pollami (`POLL_INTERVAL=8s`). `fmtTime` używa `Math.round` (nie `Math.ceil` — ceil dla ujemnych daje 0 dla -30s).
