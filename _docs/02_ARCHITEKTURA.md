...
| `kds/engine.php` | Router KDS (`get_board` z opcjonalnym `station` — filtr linii wg `sh_kds_tickets` / menu, zwraca `kitchen_delta` + `edited_since_print` + `sla_thresholds`; `bump_order` — reset `kitchen_delta`/`edited_since_print` przy przejściu do `ready`; `recall_order`; `ack_changes` — potwierdzenie zmian przez kuchnię: czyści `kitchen_delta` + `edited_since_print`, tenant-scoped). ~~`bump_ticket`~~ usunięte 2026-08-02 (orphan, drift D3 — `core/KdsTicketEngine.php` też usunięty). |
...
| `core/PromisedTimeEngine.php` | ✅ **DOMKNIĘTE 2026-07-29 (Faza B)** ⚠️ **AUDYT 2026-08-03: częściowe → 2026-08-24: L2+L3 domknięte, edycja UI dodana** — pełny silnik estymacji promised_time (load factor, channel buffers, business hours). Wpięty w 4 ścieżki ASAP: online/gateway/choiceqr/pos-accept. Scheduled: 2026-08-24 (PR #63) online checkout wywołuje `calculate('scheduled')` (L2 domknięte) + publiczny wrapper `estimate_time` (L3 domknięte). **Edycja parametrów z UI (2026-08-24, PR #65):** `base_prep_minutes` (5–120) i `min_lead_time_minutes` (15–240) edytowalne w Online Studio → Storefront. **Luki pozostałe (audyt 2026-08-03):** (L1) POS "ZAAKCEPTUJ" wysyła `now` → silnik ASAP nie odpala; (L4) gateway scheduled duplikat logiki. Szczegóły: `sessions/2026-08-03_promised_time_wiring_audit.md`, `sessions/2026-08-24_storefront_prep_time_settings.md`. |
...
| `reports/food_cost.php` | ✅ **DOMKNIĘTE 2026-07-29 (Faza D)** — food cost + margin per item (FoodCostEngine: AVCO, składniki, modyfikatory, marża per kanał). Wpięte w `modules/backoffice/food_cost/` (`food_cost_app.js` → `GET food_cost.php?item_sku=&warehouse_id=`). Pickera: `warehouse/warehouse_list.php` + `backoffice/api_menu_studio.php#get_menu_tree`. |
