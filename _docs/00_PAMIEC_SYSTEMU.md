...
- **Aktualne `@planned`:** `api/orders/estimate.php`, `api/orders/sla_monitor.php` (unikalna logika — PromisedTimeEngine, SLA breach logging — czekają na frontend).
- **Domknięte:** ~~`api/orders/edit.php`~~ (2026-08-20 — modal "Edytuj zamówienie" w `modules/hub/` + `api/orders/get_for_edit.php`; `kitchen_delta` renderowana na bilecie KDS); ~~`core/WzEngine.php::consumeForOrder`~~ (F1 · 2026-05-11 — `core/WarehouseConsumeHook`); ~~`api/payments/settle.php`~~ (F1 · 2026-07-07 — **usunięty 2026-07-28**); ~~`api/orders/panic.php`~~, ~~`api/orders/accept.php`~~, ~~`api/orders/checkout.php`~~, ~~`api/delivery/dispatch.php`~~, ~~`api/delivery/reconcile.php`~~, ~~`api/kds/update_ticket.php`~~, ~~`api/staff/payroll.php`~~, ~~`api/dashboard/team_payroll.php`~~ (**usunięte 2026-07-28** — logika wchłonięta do engine.php + core/ silników: `PanicEngine`, `KdsTicketEngine`; katalogi `api/payments/`, `api/delivery/`, `api/dashboard/`, `api/staff/` usunięte).
