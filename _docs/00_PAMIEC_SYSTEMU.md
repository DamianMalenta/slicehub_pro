...
- **Aktualne `@planned`:** `api/orders/estimate.php` (wrapper HTTP na `PromisedTimeEngine::calculate()` — docelowy konsument: storefront scheduled-picker UI. Silnik wpięty bezpośrednio w 4 ścieżki ASAP w Fazie B, wrapper pozostaje dla future pre-check UI. Jeden świadomy orphan z udokumentowanym uzasadnieniem). **⚠️ AUDYT 2026-08-03:** tryb `scheduled` `PromisedTimeEngine` również pozostaje bez konsumenta — online checkout zapisuje surowy `requested_time` zamiast wywołać `calculate('scheduled')` (patrz `sessions/2026-08-03_promised_time_wiring_audit.md` L2/L3).
- **Domknięte (Fazy A→E, 2026-07-29/30):** ~~`api/orders/edit.php`~~ (Faza E — edycja zamówienia + `DeltaEngine` → `kitchen_delta` JSON dla KDS; konsument `modules/backoffice/order_edit/` oraz rozszerzenie 2026-08-20: modal "Edytuj zamówienie" w `modules/hub/` + `api/orders/get_for_edit.php` + obsługa zmiany `order_type` z `delta.order_type {old,new}`); ~~`api/orders/sla_monitor.php`~~ (Faza C — SLA breach monitor; konsument `worker_sla_monitor.php` cron + `courses/engine.php#get_sla_breaches` + Dispatcher panel); ~~`core/PromisedTimeEngine.php`~~ (Faza B — silnik estymacji `promised_time`; wpięty w 4 ścieżki ASAP: online/gateway/choiceqr/pos-accept. ⚠️ **AUDYT 2026-08-03: częściowe domknięcie** — L1: POS "ZAAKCEPTUJ" wysyła `now` → silnik nie odpala; L3: tryb `scheduled` = martwy kod. Szczegóły: `sessions/2026-08-03_promised_time_wiring_audit.md`); ~~`api/reports/food_cost.php`~~ (Faza D — Food Cost Report; konsument `modules/backoffice/food_cost/`).
...
### `core/PromisedTimeEngine.php` — estymacja promised_time (Faza B · 2026-07-29 DOMKNIĘTE ⚠️ AUDYT 2026-08-03: częściowe → 2026-08-24: L2+L3 domknięte, edycja UI dodana)
...
- **Scheduled orders** — 2026-08-24 (PR #63): online checkout wywołuje `calculate('scheduled')` z walidacją lead-time + business-hours (L2 domknięte). Publiczny wrapper `api/online/engine.php#estimate_time` (L3 domknięte). Gateway nadal duplikuje logikę inline (L4 otwarte, niski priorytet).
- **Edycja parametrów z UI (2026-08-24, PR #65):** `base_prep_minutes` (5–120 min) i `min_lead_time_minutes` (15–240 min) są teraz edytowalne w Online Studio → zakładka Storefront → sekcja „Czy zamówień (PromisedTime)". Backend: `api/online_studio/engine.php` (`storefront_settings_get`/`save`), zapis atomowy w transakcji z `SELECT ... FOR UPDATE`. Frontend: `modules/online_studio/js/tabs/storefront.js`. Wcześniej te kolumny były ustawiane wyłącznie przez skrypty seed/install.
...
- **⚠️ AUDYT 2026-08-03 — luki w wpięciu:** (L1) POS "ZAAKCEPTUJ" w pulse sidebar wysyła `now` jako `custom_time` → silnik ASAP w `accept_order` nie odpala (ręczny input wygrywa, ale `now` nie jest "ręczny" tylko "natychmiast"); ~~(L2) online checkout scheduled zapisuje surowy `requested_time` bez wywołania `calculate('scheduled')` — brak lead-time i business-hours gate~~ **DOMKNIĘTE 2026-08-24 (PR #63)**; ~~(L3) tryb `scheduled` silnika = martwy kod (0 produkcyjnych call-site'ów, jedyny konsument to orphan `estimate.php`)~~ **DOMKNIĘTE 2026-08-24 (PR #63)**. Szczegóły: `_docs/sessions/2026-08-03_promised_time_wiring_audit.md`.
- Szczegóły Fazy B: `_docs/sessions/2026-07-29_phase_b_promised_time_engine.md`. Szczegóły edycji UI: `_docs/sessions/2026-08-24_storefront_prep_time_settings.md`.
...
- **Cel:** warstwa CONCRETE PROVIDER między `sh_event_outbox` a konkretnymi API (Papu.io, Dotykačka, GastroSoft, Elzab, ChoiceQR, …). Uzupełnia `WebhookDispatcher` (generyczne HTTP) o mapowanie per-provider: payload shape, auth headers, response semantics.
...
- **Registry (`AdapterRegistry`):** `PROVIDER_MAP` mapuje `sh_tenant_integrations.provider` → klasa PHP (`papu` → PapuAdapter, `dotykacka` → DotykackaAdapter, `gastrosoft` → GastroSoftAdapter, `elzab` → ElzabAdapter, `choiceqr` → ChoiceQRAdapter). Cache per tenant_id. Feature-detect fallback gdy health columns brakują.
- **Adaptery:** PapuAdapter (Bearer + opcjonalny HMAC `X-Papu-Signature`), DotykackaAdapter (OAuth2 Bearer z refresh flow, MVP używa raw refresh_token), GastroSoftAdapter (X-Api-Key, 409 Conflict = idempotency success), ElzabAdapter (drukarka fiskalna — patrz sekcja Elzab niżej), ChoiceQRAdapter (token auth `?t=SECRET`, odwrócony model — oni pushują do nas przez dedykowane endpointy, patrz Inbound Callbacks).
...
### Inbound Callbacks — `api/integrations/inbound.php` + `BaseAdapter::parseInboundCallback()` (Faza 7 · sesja 7.6 · P2.1 2026-07-29)
...
- **Flow:** INSERT log → lookup integrations + decrypt credentials → `AdapterRegistry` → `adapter.parseInboundCallback($raw, $headers, $creds)` → verify signature + map status → idempotency check `UNIQUE(provider, external_event_id)` → match `external_ref` → `gateway_external_id` → **`OrderStateMachine::transitionOrder()`** (od P2.1, z `SELECT ... FOR UPDATE` w transakcji) + opcjonalnie `delivery_status` (`canTransitionDelivery`) + `payment_status` (bezpośredni UPDATE z `tenant_id` w WHERE) → `OrderEventPublisher::publishOrderLifecycle()` → UPDATE `sh_inbound_callbacks.status='processed'`.
- **Adapter kontrakt:** `BaseAdapter::supportsInbound(): bool` (default false), `BaseAdapter::parseInboundCallback($rawBody, $headers, $credentials)` → `{ok, signature_verified, external_event_id, external_ref, event_type, new_status, new_delivery_status, new_payment_status, payload, error?}`.
...
- **Status transitions (P2.1):** `inbound.php` nie używa hardcoded whitelist — wywołuje `OrderStateMachine::transitionOrder()` który waliduje przejścia według `STRICT_TRANSITIONS` (lub `DINE_IN_TRANSITIONS` dla `order_type='dine_in'`), rozszerza mapę o feature flagi (`skip_kitchen`, `auto_complete`, …), wykonuje `SELECT ... FOR UPDATE` i zapisuje audit trail do `sh_order_audit` + `sh_order_logs`. Cofnięcie statusu zablokowane.
...
- **ChoiceQR (P2.1, dedykowane endpointy):** ChoiceQR ma **odwrócony model** (oni pushują do nas) i własne endpointy zamiast `inbound.php`: `api/integrations/choiceqr/events.php` (webhook eventy: status/delivery/QR payment/menu), `api/integrations/choiceqr/pay.php` (potwierdzenie płatności QR przy stoliku). Auth: token w `?t=SECRET_TOKEN`, tenant mapping przez `varSymbol`. Response: **200 OK empty body** (ChoiceQR anuluje zamówienie po 3 brakach 200). Log-only events (section/category/dish changes, marketplace.acceptance) → 200 OK bez zmian w DB.
...
### `core/Elzab/` — bezpośrednia fiskalizacja Elzab Zeta Online (2026-07-29 · smart switch 2026-07-29b)
- **Cel:** bezpośredni druk paragonu fiskalnego przez protokół Thermal over TCP (port 1001) — bez pośrednika, bez ESC/POS bridge. Numer paragonu z drukarki → `sh_orders.fiscal_receipt_number` (migracja 062).
- **Komponenty:** `ElzabFiscalEngine::fiscalizeOrder(pdo, orderId, tenantId, userId, force=false)` — orchestracja (pobierz order → guard → pobierz linie → drukuj → zapisz numer), `ElzabPrinter` — sterownik TCP (`stream_socket_client`, komendy Thermal: `$h` handshake, `$l×N` linie, `$y` end transaction + szuflada).
- **Guard podwójnej fiskalizacji:** jeśli `fiscal_receipt_number` już istnieje → blokuj (chyba że `force=true` dla reprintu). POS wywołuje reprint z `force=true` po `confirm()`.
- **Smart switch (POS):** `_checkFiscalStatus()` przy starcie → `_fiscalReady` flag. Jeśli drukarka online → drukuj **wyłącznie fiskalny** (settle, karta, print-only modal). Jeśli offline → fallback na niefiskalny (`window.print`). Na karcie zamówienia jeden przycisk (🧾 fiskalny / 📄 niefiskalny) zamiast dwóch.
- **Konfiguracja:** w module Settings (zakładka „Drukarka Fiskalna") — IP, port, cashbox, stopka paragonu + test połączenia + druk paragonu testowego (`fiscal_test_print`).
- **Raport dobowy:** `fiscal_daily_report` z poziomu POS topbar.
- **Adapter integracji:** `core/Integrations/ElzabAdapter.php` zarejestrowany w `AdapterRegistry` (provider `elzab`).
- Szczegóły: `_docs/audits/fiscalization_status.md`, `_docs/16_RESILIENT_POS.md`.
