 ...
 *   php scripts/worker_payroll_accrual.php --loop --sleep=5
 *
 * Exit codes:
 *   0 — OK (batch processed)
...
if (!isset($pdo) || !($pdo instanceof PDO)) {
    fwrite(STDERR, "[worker_payroll_accrual] FATAL: \$pdo not available from db_config.php\n");
    exit(1);
}
...
foreach ($argv ?? [] as $arg) {
    ...
    if (preg_match('/^--sleep=(\d+)$/', $arg, $m)) {
        $sleepSec = max(1, (int)$m[1]);
    }
}

// ── 3. PID lock (prevent duplicate workers on same node) ─────────────────
$pidDir = dirname(__DIR__) . '/logs';
if (!is_dir($pidDir)) {
    @mkdir($pidDir, 0775, true);
}
$pidFile = $pidDir . '/worker_payroll_accrual.pid';
$pidHandle = @fopen($pidFile, 'c+');

if (!$pidHandle) {
    fwrite(STDERR, "[worker_payroll_accrual] FATAL: cannot open PID file {$pidFile}\n");
    exit(1);
}
...
if (!flock($pidHandle, LOCK_EX | LOCK_NB)) {
    ...
    fclose($pidHandle);
    fwrite(STDERR, "[worker_payroll_accrual] another instance is running (pid={$existingPid}) — exiting\n");
    exit(2);
}
...
do { // Loop wrapper (single-pass unless --loop)
...
echo "[payroll_accrual] batch: " . json_encode($stats) . "\n";

if ($loopMode) {
    sleep($sleepSec);
}

} while ($loopMode); // end do/while loop
